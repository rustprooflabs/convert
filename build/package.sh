...
VERSION=$(cat convert.control | grep default_version | cut -f2 -d\')

echo "Convert Building for:  ${OSNAME}-${VERSION}"
...
#
cd target/release/convert-${PG_VER} || exit $?
...
#
OUTNAME=convert_${VERSION}_${OSNAME}_${PG_VER}_amd64
